<div wire:key="{{ $todo->id }}"
    class="container mx-auto mb-5 card px-5 py-6 bg-white col-span-1 border-t-2 border-purple-500 hover:shadow">
        ...
        <div class="flex items-center">
            ...
            <div>
                <input wire:model="editTodoName" type="text" placeholder="Todo.."
                    class="bg-gray-100 text-gray-900 text-sm rounded block w-full ms-2 p-2.5">
                @error('editTodoName')
                <span class="mt-1 ms-2 text-red-500 text-xs block">{{ $message }}</span>
                @enderror
            </div>
            @else
            <h3 class="text-lg text-semibold text-gray-800 ms-2">{{ $todo->name }}</h3>
            @endif
    ...
    <div class="mt-3 text-xs text-gray-700">
        ...
        <button wire:click="cancelEdit"
            class="mt-3 ms-2 px-4 py-2 bg-red-500 text-white font-semibold rounded hover:bg-red-600">Cancel</button>
        @endif
    </div>
</div>